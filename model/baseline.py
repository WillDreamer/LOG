######### IJCAI19 One Network for Multi-Domains: Domain Adaptive Hashing with Intersectant Generative Adversarial Networks
import torch
...
def train(train_s_dataloader,
    ...
    model.to(device)
    # model = network.VGGFc("VGG16",code_length)
    # model.to(device)
    ...
    for epoch in range(max_iter):
        # if epoch > 0 and (epoch-1) % 10 ==9:
        #     threshld1 = sorted(w_list)[int(0.4*len(w_list))]
        #     threshld2 = sorted(w_list)[int(0.2*len(w_list))]
        #     w_list = []
        for ((data_s, target_s, index), (data_t, target_t_gt, _)) in\
            ...
            q = q.to(device)
            w = js_div(p,q)
            # if epoch > 0 and (epoch-1) % 10 == 8:
            #     w_list.append(w)

            # office 0.25 0.23

            if epoch >= 29 and epoch < 39 and w < 0.03:
                # Target pseudo ce loss
                loss_t,_ = criterion_new(logit_t, code_t, target_t, onehot=False)
                total_loss = transfer_loss + loss_s + align_loss + 0.1 * loss_t
            
            elif epoch >= 39 and epoch < 49 and w < 0.02:
                # Target pseudo ce loss
                loss_t,_ = criterion_new(logit_t, code_t, target_t, onehot=False)
                total_loss = transfer_loss + loss_s + align_loss + 0.1 * loss_t
            
            else:
                total_loss = transfer_loss + loss_s + align_loss
